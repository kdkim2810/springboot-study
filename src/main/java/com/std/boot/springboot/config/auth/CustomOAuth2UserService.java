...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        /*
        OAuth2 로그인 진행 시 키가 되는 필드값들을 이야기함(Primary Key와 같은 의미)
        ...
        이후 네이버 등 다른 소셜 로그인도 해당 클래스를 사용
         */
        ...
        User클래스를 사용하지 않는 이유는 직렬화(Serialize)를 구현하지 않아서 에러가 나기 때문
        -> User 클래스를 직렬화를 하기에는 User 클래스가 엔티티이고 만약 @OneToMany, @ManyToMany 등 자식 엔티티를 가지고 있다면 직렬화 대상에
        자식들도 포함되므로 성능 이슈, 부수 효과가 발생할 확률이 높음(직렬화를 상속받아야 하기 때문)
         */
